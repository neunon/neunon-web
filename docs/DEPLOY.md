...
実名・メールアドレス・時間単価などは公開HTMLへ渡さず、自動採番の学生No、
大学名、学部・研究科、学年、週稼働可能時間、スキル、対応領域、案件経験数から匿名の公開プロフィールを生成する。
同期時に「学生No」列がなければ作成し、空欄の登録者に重複のない5桁の番号を付番してリストへ保存する。
同時に、掲載可の公開項目だけを `content/talent/talents.generated.json` へ書き出し、Renderはその確定済みスナップショットからビルドする。
...
動きは CSS アニメーションだけで作ってあるので、JS が無くても
本文は最初から見えているはずだが、実機で確かめる。

- [ ] 本文がすべて読める
- [ ] ヘッダーとフッターのリンクをたどれる
- [ ] ヘッダーのドロップダウンとハンバーガーメニューは開かないが、
      各ページへのリンクはフッターからたどれる
